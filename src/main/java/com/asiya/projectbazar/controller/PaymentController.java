...
public class PaymentController {
	...
	public String payWithEsewa(@PathVariable int id, Principal p, Model m) {
		...
		payment.setPdc(0);
		payment.setPsc(0);
		payment.setTotal_amount(Math.ceil(amount));
		payment.setProduct_code("EPAYTEST");
		payment.setTransactionalUUID(UUID.randomUUID()+"-0C81");
		payment.setSigned_field_name("total_amount,transaction_uuid,product_code");
		String message = "total_amount=" + payment.getTotal_amount() + ",transaction_uuid="
				+ payment.getTransactionalUUID() + ",product_code=" + payment.getProduct_code();
